...
block() {
  printf 'BLOCKED for frontend agent: %s Finish the frontend work and name exactly what should run in your report — qa runs it and sends results back.\n' "$1" >&2
  exit 2
}
...
if printf '%s' "$cmd" | grep -Eiq '(^|[^[:alnum:]_-])(playwright|cypress|vitest|jest|mocha|ava|karma|stryker|nyc|c8)([[:space:]]|$)'; then
  block "test/e2e/coverage runners are qa's job."
fi
...
if printf '%s' "$cmd" | grep -Eiq '(^|[^[:alnum:]_-])(pytest|hyperfine|k6|artillery|autocannon|lighthouse)([[:space:]]|$)'; then
  block "test and benchmark tools are qa's job."
fi
if printf '%s' "$cmd" | grep -Eiq '(^|[^[:alnum:]_-])(go|cargo|mvn|gradle|\./gradlew)[[:space:]]+(test|bench)([[:space:]]|$)'; then
  block "language-level test/bench commands are qa's job."
fi
